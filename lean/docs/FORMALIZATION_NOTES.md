...
The contribution is pinned to arXiv:2312.05481v11. `SourceModel.lean`
formalizes the paper's knowledge distribution, hierarchy technology, five firm
configurations, compute accounting, matching resource constraint, competitive
equilibrium, output/income, and comparative-static vocabulary.

`PaperInterface.lean` contains one transparent specification for each of
Propositions 1–6, and `ProofInterface.lean` exposes the exact paired theorem
types. The current status is partial because the six source-level analytical
proofs remain explicit in `MainTheorems.lean`.

The current paper target has an authoritative successful 2,501-job build with
exactly those six `sorry` warnings. The draft semantic preflight subsequently
refused the first four-Spec bounded display batch. Accepting closeout is also
blocked before graph acquisition because the planner requires a registered
formalization-engine revision transition involving state outside this paper
folder. These are recorded workflow blockers; no final semantic receipt,
validation report, or dependency DAG has been manufactured around them.
The focused paper-contribution check nevertheless passes its PaperInterface
build, semantic import-isolation, and scoped diff phases.

The remaining paper-local gates were run only once. Conclusion and assumption
provenance failed closed for missing current evidence/source-record audits.
Statement and paper-coverage inventory each timed out after 300 seconds with
zero Lean records, and source-to-Lean produced no verdict before its stale
worker was stopped. The existing evidence-integrity result likewise remains
failed closed because no planner-issued import-closure/source-route evidence
exists. None of these outcomes is represented as semantic evidence or a pass.
...
- Version formalized: arXiv:2312.05481v11
...
|---|---|---|---|---|
| No source-pinned targets supplied | `none` | not started | `none` | Do not add a generic `True` theorem; extract exact source statements first |
